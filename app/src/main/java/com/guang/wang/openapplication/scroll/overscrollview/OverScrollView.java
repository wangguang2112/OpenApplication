...
public class OverScrollView extends FrameLayout implements OnTouchListener {
    ...
    public OnMyScrollChangeListener mListener;
     ...
     * 向下滑动的最大距离，初始为500pix
     *
     * @param maxOverGap pix
     */
    public void setMaxOverGap(int maxOverGap) {
        if(headView!=null||footView!=null){
            throw new IllegalStateException("MaxOverGap must set behind of header or footer");
        }
       initChildPointer();
    }
    ...
    private void initBounce() {
        ...
        overScrollerSpringbackTask = new Runnable() {
            @Override
            public void run() {
                // scroll till after the paddingi
                mScroller.computeScrollOffset();
                scrollTo(0, mScroller.getCurrY());
//				scrollTo(0, getMaxScrollAmount());
//				Log.d("wang",mScroller.getCurrY()+"");
    ...
    public void initChildPointer() {
        child = getChildAt(0);
        child.setPadding(0, maxOverGap, 0, maxOverGap);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsBeingDragged) {
                    ...
                    if (isOverScrolled()) {
                        ...
                        scrollBy(0, deltaY / overScrolledStrength());
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(!isOverScrolled()&&mListener!=null){
                     mListener.onScrollOver(maxOverGap,0,getScrollY());
                }
                break;
            case MotionEvent.ACTION_CANCEL:
     ...
     * 确定拉伸的强度
     */
    public int overScrolledStrength() {
        int off = 0;
        if (getScrollY() < child.getPaddingTop()) {
            off = child.getPaddingTop() - getScrollY();
        } else if (getScrollY() > child.getBottom() - child.getPaddingBottom() - getHeight()) {
            off = getScrollY() - child.getBottom() + child.getPaddingBottom() + getHeight();
        } else {
            return 0;
        }
        int strength = off * 4 / maxOverGap;
        return strength == 0 || strength == 1 ? 2 : strength;
    }
    ...
    protected void onScrollChanged(int leftOfVisibleView, int topOfVisibleView, int oldLeftOfVisibleView, int oldTopOfVisibleView) {
        if (mListener != null) {
            mListener.onMyScrollChanged(maxOverGap, leftOfVisibleView, topOfVisibleView, oldLeftOfVisibleView, oldTopOfVisibleView);
        }
    ...
    protected int onOverScroll(int scrollY) {
        Log.d(TAG, "onOverScroll() called with: scrollY = [" + scrollY + "]");
        return 0;
    }

    public void setOnMScrollListener(OnMyScrollChangeListener listener) {
        mListener = listener;
    }
     ...
     * 添加头部view
     */
    public void addOverHeadView(Context context, View view, LinearLayout.LayoutParams params) {
        LinearLayout head = new LinearLayout(context);
        int height = 0;
        if (params.height == ViewGroup.LayoutParams.MATCH_PARENT) {
            params.height = maxOverGap;
            height = 0;
        } else {
            int mh;
            if (params.height == ViewGroup.LayoutParams.WRAP_CONTENT) {
                mh = View.MeasureSpec.makeMeasureSpec(0,
                        MeasureSpec.UNSPECIFIED);
            } else {
                mh = View.MeasureSpec.makeMeasureSpec(params.height,
                        MeasureSpec.EXACTLY);
            }
            view.measure(0, mh);
            mh = view.getMeasuredHeight(); // 获取高度
            height = mh < maxOverGap ? maxOverGap - mh : 0;
        }
        params.setMargins(0, height, 0, 0);
        head.addView(view, params);
        super.addView(head, -1, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, maxOverGap));
        headView = view;
    }
    /**
     * 添加头部view
     */
    public void addOverFootView(Context context, View view, LinearLayout.LayoutParams params) {
        LinearLayout head = new LinearLayout(context);
        int  mh = View.MeasureSpec.makeMeasureSpec(0,
                MeasureSpec.UNSPECIFIED);
        this.getChildAt(0).measure(0, mh);
        int height =  this.getChildAt(0).getMeasuredHeight()-maxOverGap;
        Log.d(TAG,"foot"+height+"");
        params.setMargins(0, height, 0, 0);
        head.addView(view, params);
        LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, maxOverGap);
        super.addView(head, -1, pp);
        footView = view;
    }
}
